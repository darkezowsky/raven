...
float ComputeHeight(float3 wpos, float waterLevel)
{
    ...
    float k = (FdotC <= 0.0f ? 1.0 : 0.0); // Is camera below height fog
    float FdotP = P.y - FH;
    float FdotV = wsDir.y;
...
{
    // Planar depth projection for caustics
    float3 caustics1 = tex2D(sampler_CausticsTex, depthPos.xz * tiling + time.xy).rgb;
    float3 caustics2 = tex2D(sampler_CausticsTex, depthPos.xz * tiling * 0.8 - time.xy).rgb;
